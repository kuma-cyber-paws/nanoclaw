...
async function collectAgentGroups() {
  ...
  return Promise.all(
    groups.map(async (g) => {
      ...
      const memberRows = await getMembers(g.id);
      ...
      const adminRows = await db.all<Record<string, unknown>>(
        `SELECT ur.user_id, ur.role, u.display_name
         FROM user_roles ur
         LEFT JOIN users u ON u.id = ur.user_id
         WHERE ur.agent_group_id = ? AND ur.role = 'admin'`,
        g.id,
      );
...
async function collectChannels() {
  ...
  for (const mg of messagingGroups) {
    ...
    const agentRows = await getMessagingGroupAgents(mg.id);
